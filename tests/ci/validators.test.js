...
function runTests() {
  ...
  if (test('accepts the documented match-all matchers "*" and ""', () => {
    // Claude Code resolves both to match-all before regex evaluation, so they
    // must pass even though `new RegExp('*')` throws. A fork commit once
    // rejected "*" as an invalid regex; this pins the correction.
    const testDir = createTestDir();
    ...
    assert.strictEqual(result.code, 0, `Should accept match-all matchers, got stderr: ${result.stderr}`);
    cleanupTestDir(testDir);
  })) passed++; else failed++;

  if (test('fails on a matcher that is not a valid regular expression', () => {
    const testDir = createTestDir();
    const hooksFile = path.join(testDir, 'hooks.json');
    fs.writeFileSync(hooksFile, JSON.stringify({
      hooks: {
    ...
    const result = runValidatorWithDir('validate-hooks', 'HOOKS_FILE', hooksFile);
    assert.strictEqual(result.code, 1, 'Should fail on an uncompilable matcher');
    assert.ok(result.stderr.includes('not a valid regular expression'), 'Should name the problem');
    cleanupTestDir(testDir);
  })) passed++; else failed++;
