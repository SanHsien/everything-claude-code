...
> **SanHsien 維護型 fork。** 產品漏洞（skills、agents、安裝器、npm `ecc-universal`）仍請用下文的上游 private reporting：[`affaan-m/ECC` advisories](https://github.com/affaan-m/ECC/security/advisories/new)。本 fork overlay（`tools/`、`docs/fork/`、workflow guard）開 [`SanHsien/everything-claude-code`](https://github.com/SanHsien/everything-claude-code) 的 Security tab。維護規則見 [`FORK.md`](FORK.md)。本線不發 npm，不承擔官方 SLA。
