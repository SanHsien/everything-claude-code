...
// Matcher values Claude Code treats as match-all without evaluating them as regex.
    ...
    for (const [eventType, matchers] of Object.entries(hooks)) {
      ...
      for (let i = 0; i < matchers.length; i++) {
        ...
        if (!('matcher' in matcher) && !EVENTS_WITHOUT_MATCHER.has(eventType)) {
          ...
          hasErrors = true;
        } else if (typeof matcher.matcher === 'string' && !MATCH_ALL_MATCHERS.has(matcher.matcher)) {
          // Claude Code resolves `*` and `""` to match-all *before* any regex
          // evaluation (https://code.claude.com/docs/en/hooks, matcher table),
          // so those two are valid even though `new RegExp('*')` throws. Every
          // other string is either an exact-name list or an unanchored
          // JavaScript regex; an exact-name list always compiles, so compiling
          // is the right check for the rest. A fork commit once rejected `*`
          // here on the false premise that it was an invalid regex -- keep the
          // special case, and keep the test that pins it.
          try {
            new RegExp(matcher.matcher);
          } catch (error) {
            console.error(
              `ERROR: ${eventType}[${i}] matcher ${JSON.stringify(matcher.matcher)} is not a valid regular expression: ${error.message}. For match-all use "*", "" or omit the matcher.`,
            );
            hasErrors = true;
          }
        }
